...
def exp(cfg: DictConfig, save_path: str, agent: Agent, wandb_cfg = None):
    ...
    with wandb.init(config=wandb_cfg):
        ...
        cfg["train_alg"] = wandb_cfg.algorithm
...
def main(cfg: DictConfig) -> None:
    ...
    sweep_id = wandb.sweep(sweep_config, project=f"dcmh_alg_search {cfg['dataset']} {cfg['test_alg']} tau_{cfg['tau']} beta_{cfg['beta']}")
    wandb.agent(sweep_id, function=lambda: exp(cfg, save_path, agent))
